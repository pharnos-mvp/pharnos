...
- **N2-b (PERF) LIVRÉ EN PROD** : code-split `DossierWorkspacePage` — `RichTextEditor` (TipTap) + `TemplateFillForm` en `React.lazy`/`Suspense` ([#172](https://github.com/pharnos-mvp/pharnos/pull/172)) → **route workspace 183 → 41 Ko gzip (−77%)**, TipTap chargé seulement à l'édition, offline-safe. Budget de route ≤ 70 Ko ajouté ; **Lighthouse a11y ≥ 0.95 + best-practices ≥ 0.9 BLOQUANTS** (perf reste advisory : CI mobile throttlé **= 0.65**, gate perf déterministe = budget bundle — décision CEO). e2e offline (montage → route rendue hors-ligne + chunks éditeur précachés). 235 unit + 15 e2e verts. ⏳ **recette navigateur CEO en prod** (éditeur/formulaire/aperçu/compile, en ligne + hors-ligne). **Note plan** : la cible « Lighthouse perf ≥ 90 en CI » est INGATEABLE sur runner mobile throttlé (0.65) → remplacée par le budget bundle (déterministe) ; perf terrain validée à la recette.
- **N3 (SCALABILITÉ) — gros œuvre LIVRÉ EN PROD** : **N3-a** index composites `(org_id, updated_at)` sync `0035` ([#174](https://github.com/pharnos-mvp/pharnos/pull/174), EXPLAIN prouvé : Index Scan, pas de Sort) · **N3-b-1** backstop stockage serveur `0036` (bucket 50 Mo + MIME allowlist + helper `contentTypeFor`, [#175](https://github.com/pharnos-mvp/pharnos/pull/175)) · **N3-b-2** **quota stockage administrable en god mode** `0037` ([#176](https://github.com/pharnos-mvp/pharnos/pull/176)) : `max_storage_bytes` free 1/pro 20/team 50/business 100/ent ∞ Go + RPC usage par org + jauge Compte→Abonnement + édition `/admin` (Plans + override) ; Edge admin redéployé ; fix `team` éditable en admin. **N3 COMPLET (2026-06-17)** : **k6 léger** Edge ([`load/k6-edge-gates.js`](load/k6-edge-gates.js), [rapport](load/REPORT.md)) — 8+8 VUs, 948 req sur les chemins de rejet **sans Gemini** : gardes auth 401/403 + public `share` 4xx/429 **100 % rejetées, 0 fuite, 0 5xx, p95 < 0,5 s** ; **doc capacité par palier** [`docs/CAPACITY-N3.md`](docs/CAPACITY-N3.md) (goulot = Storage ~50–70 dossiers réels sur Free ; DB 20 Mo/4 % ; déclencheurs bascule Pro). Le modèle de **features 3 états** (god mode, migration `0038`, PR #179) a été livré en parallèle.
- **RESTE** : **N4** (gate signé + recette 3 pilotes + **bascule Supabase Pro** : leaked-password ON + `auth.pharnos.com` pour le branding « Pharnos » sur l'écran Google + **quota stockage DUR D2** au moment Pro).
- **Migrations : dernier appliqué = `0037` → reprendre à `0038`.** Runbook secrets : [SECRETS-ROTATION.md](SECRETS-ROTATION.md).
...
## N3 — Scalabilité · ~0,5 session · **1 PR + 1 artefact** · ✅ **COMPLET (2026-06-17)**
> EXPLAIN sync indexé (`0035`) · backstop stockage (`0036`) + quota administrable (`0037`) · **k6 gates verts**
> ([`load/`](../load/)) · **capacité documentée** ([CAPACITY-N3.md](CAPACITY-N3.md)). Reste **N4**.
