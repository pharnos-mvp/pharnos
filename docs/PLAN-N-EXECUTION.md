...
- **N3 (SCALABILITÉ) — gros œuvre LIVRÉ EN PROD** : **N3-a** index composites `(org_id, updated_at)` sync `0035` ([#174](https://github.com/pharnos-mvp/pharnos/pull/174), EXPLAIN prouvé : Index Scan, pas de Sort) · **N3-b-1** backstop stockage serveur `0036` (bucket 50 Mo + MIME allowlist + helper `contentTypeFor`, [#175](https://github.com/pharnos-mvp/pharnos/pull/175)) · **N3-b-2** **quota stockage administrable en god mode** `0037` ([#176](https://github.com/pharnos-mvp/pharnos/pull/176)) : `max_storage_bytes` free 1/pro 20/team 50/business 100/ent ∞ Go + RPC usage par org + jauge Compte→Abonnement + édition `/admin` (Plans + override) ; Edge admin redéployé ; fix `team` éditable en admin. **RESTE N3** : **k6 léger** Edge (gates 401/403/429 + p95, sans brûler de Gemini) + **doc capacité par palier**.
