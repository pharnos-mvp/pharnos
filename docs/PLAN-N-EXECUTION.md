...
- **RESTE** : **N3** (EXPLAIN/k6 + **D1 stockage** : garde-fous fichier + jauge usage par org — cf. [PLAN-DATA-STORAGE-QUOTA.md](PLAN-DATA-STORAGE-QUOTA.md)) · **N4** (gate signé + recette 3 pilotes + **bascule Supabase Pro** : leaked-password ON + `auth.pharnos.com` pour le branding « Pharnos » sur l'écran Google + quota stockage dur D2).
