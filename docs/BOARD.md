...
_Dernière mise à jour : **2026-07-08** — **🔔 RELANCES domaine B COMPLET + recette prod Chrome passée (les 3 tranches 1b/2a/2b vérifiées bout-en-bout, 2 e-mails délivrés) + lifecycle M3–M6+CS1 + LOTs 7-8-9 + LOT 10 (relances auto + v3 + M7) + centre de notifications + relances langue-par-rôle & heures ouvrables (#309) EN PROD ; migrations à `0059` (reprendre `0060`), Dexie v15 (16 libre). Voir la ligne consolidée #259→#309 du §10 + la mémoire `reminders-notifications`. Reste vers GO-LIVE : LOT 11 landing → LOT 12 i18n+M4 → LOT 13 recette → LOT 14 (= N4).** _(Historique 2026-07-02 :)_ **Jalons H→O + pivot P1 + refonte CTD builder + Bibliothèque Templates 5/5 + 🚀 Moteur de Variation + LOT 2 Catalogue RIM COMPLET (#252→#258) + Workspace cockpit v2 (#264) + 🆕 LIFECYCLE « la spine » M0–M2 (#272→#278 : journal append-only `0047` + `deriveLifecycle` 7 étapes + Roadmap parcours + actions Labo) EN PROD ; migrations à `0047` (reprendre à `0048`) ; gate N : N1✅ N2✅ N3✅ → reste N4, pilote #1 EN COURS (Bénin). Workflow lifecycle complet validé CEO 2026-07-02 → M3→M8 ordonnancés ([PLAN-LIFECYCLE.md](PLAN-LIFECYCLE.md) §5) **+ CS1 collaboration compte-à-compte scopée au dossier** (§5-bis, insérée M4→CS1→M5). Chemin consolidé jusqu'au lancement (lifecycle M3→M4→CS1→M5-M6 → LOTs 7-9 → LOT 10 fusionné → landing → i18n → recette → GO-LIVE) : [PLAN-LANCEMENT.md](PLAN-LANCEMENT.md).** Plan de finition « zéro-dette » : [PLAN-FINITION-ZERO-DETTE.md](PLAN-FINITION-ZERO-DETTE.md). _Bloc historique (06-17) :_ **Jalons H→M + O + N1 + N2 + N3 (gros œuvre) EN PROD.** Tracker vivant
...
**→ MAJ 2026-07-08 : lifecycle M3–M6+CS1, LOTs 7-8-9, LOT 10 (relances auto) et RELANCES domaine B (contact + moteur fabricant, RECETTE PROD PASSÉE) = TOUS EN PROD. Reste vers GO-LIVE : LOT 11 landing → LOT 12 i18n+M4 → LOT 13 recette → LOT 14 (= N4).**
...
| **#259→#309** | 06-29 → 07-08 | **Résumé consolidé** (détail = `git log` + mémoires `dossier-lifecycle`/`launch-plan-zero-debt`/`reminders-notifications`) : **LIFECYCLE « la spine » M3→M6 + CS1** (collaboration compte-à-compte scopée `0048`/`0049`) EN PROD · **LOT 10 CLOS** — **relances AUTO Roadmap** (cron nocturne 05:17 UTC `0050`/`0051`, auth secret hash Vault) + correspondance **v3** (export PDF/délai, #293) + **M7 vue Agent tokenisée** (`0052`) · **LOTs 7-8-9** — `/compte` (#295), `/admin` god-mode (#296), cockpit/Journal/recherche (#297, `0053`), **Corbeille + purge rétention 30 j serveur** (#298, `0054`, Edge `retention-purge`) · **centre de notifications (cloche Reçu/Envoyé)** (#302/#303) · **🔔 RELANCES domaine B COMPLET** : page Relances (`0055`), Slice 1a (relance adressée au destinataire + accusé, #304), **1b** langue destinataire à l'envoi + « Destinataires » (`0056`, #305), **2a** contact fabricant sur `parties` (`0057`, #306), **2b** moteur relance FABRICANT (pass monitoring isolée du cron + idempotence `monitoring_reminders` `0058`, #307). **RECETTE PROD Chrome MCP passée (07-08)** : 3 tranches vérifiées bout-en-bout, 2 e-mails délivrés · **#309 relances : langue par RÔLE** (MAH/fabricant = EN, agence = langue du pays ; fini le bilingue) **+ heures ouvrables** (cron → `0 9 * * 1-5` = 09:00 UTC lun–ven, migration `0059`) · sync docs #308/#310. Migrations → `0059` (next `0060`). **Reste vers GO-LIVE : LOT 11 landing → LOT 12 i18n+M4 → LOT 13 recette → LOT 14 (= N4).** |
