...
| **M3 ✅** | **Lettre de demande + PGHT bilingues (éditeur dédié)** | Lettres 1.1.1/1.1.2 bilingues FR/EN (additif, **FR dossier/pilote inchangé**) + **éditeur standalone Bibliothèque** : sélecteur pays (8 UEMOA) → destinataire/agence/civilité/ville **auto** ; aperçu A4 + PDF/DOCX (lazy) + Enregistrer. Renderers génériques (zéro duplication de contenu). **Bibliothèque 5/5.** **Redesign M3.1 (inline)** : A4 à **cases remplissables directement** (comme RCP/Notice) + barre d'en-tête hors-template (pays → agence **+ civilité auto** · **sélecteur produit** catalogue→auto-sync, saisie manuelle inline toujours dispo) + **nom/poste auto du profil** + **devise PGHT** (FCFA/Naira/Cedi/Dirham/Euro/USD/GBP/AUD). **Tranche 2 ✅** : insertion **1-clic en-tête/pied + signature** (images du profil) dans l'aperçu + PDF + DOCX (ImageRun). | ~1 s |
...
éditeur inline + en-tête/pied/signature 1-clic & destinataire auto par pays). **Bibliothèque 5/5.**
**Reste M4** (dernier jalon) : `submissionLanguage`
