...
**Séquence de déploiement C (exécutée dans l'ordre sûr)** : ① `0094` appliquée (RPC hash + skips + cron gardé) ② secrets Vault créés EN base (`gen_random_bytes`) ③ sonde (hash 64, table, cron posé) ④ Edge `chariow-pulse`/`chariow-reconcile`/`order-invoice`/`job-tick` déployées ⑤ `CHARIOW_PULSE_OBSERVE=1` posé ⑥ fumée 3/3 (reconcile 403 fermé · invoice 404 propre · pulse charabia 200 acquitté). **Gestes CEO restants** : copier le secret `whsec_…` (console Chariow → Pulse → Signing secret → Reveal) → `supabase secrets set CHARIOW_PULSE_SECRET=…` puis `supabase secrets unset CHARIOW_PULSE_OBSERVE` ; désactiver la clé de licence du produit en console (sinon l'e-mail n°1 l'explique déjà).
