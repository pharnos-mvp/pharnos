...
> | **Suivant** | LOTS A/B/C ✅ + gestes console ✅ + **rail prouvé par rejeu signé de la vente réelle (2026-08-15, §« Séquence C »)** — il ne reste que **LE run unique** (feu vert CEO + SON paiement 570 F via le lien de recette) → réouverture → **bundle up3** (leviers vitesse L1+L3 inclus) |
...
### LOT C + B2 — le rail — ✅ **LIVRÉ le 2026-08-15** (une seule réécriture du parcours post-paiement)
| # | Fait — verrou |
|---|---|
| ✅ C1 | **Réconciliation active** : cron 2 min (`0094`, patron Vault de 0086 — secrets créés EN base, garde `where exists` sur les secrets) → Edge `chariow-reconcile` : liste `GET /v1/sales` (fenêtre 2 j, tri par `STATUTS_PAYES` **partagée avec `lireVente`** — une liste locale a failli affamer le balayage sur `settled`), naissance par le chemin re-vérifié via **`_shared/order-birth.ts`, LE module partagé webhook/réconciliation** (machine à états épinglée par 6 tests sur client bouchonné). Filets : **table `chariow_reconcile_skips`** (une vente à jamais non-naissable ne se re-télécharge pas toutes les 2 min ni n'occupe le cap), **balayage des MUETTES** (commande née pendant une panne Resend → e-mail n°1 re-servi — Chariow a eu son 200, personne d'autre ne reviendra), **liste pleine = alerte** (le magasin partage ses 50 lignes avec les packs CTD Builder). Chaque naissance ici = alerte support (le Pulse a manqué — panne de rail, pas un état normal) |
| ✅ C2 | **Salle d'attente** : cadence portée à ~5 min 30 — **calibrée pour survivre à DEUX périodes du cron de réconciliation** (le Pulse peut ne JAMAIS arriver : l'acheteur doit être encore là quand le balayage fait naître sa commande) ; paliers de message horodatés (`PALIERS_ATTENTE`, jamais muette, ARIA repeinte au changement seulement) ; à la confirmation, **le LIEN s'affiche À L'ÉCRAN** (`#cfmlink`, 3,5 s avant l'auto-redirection) ; **marche depuis N'IMPORTE QUEL appareil** (la référence UUID seule suffit — l'ancienne impasse « commande introuvable sur cet appareil » est morte avec le fichier) |
| ✅ C3 | **Signature des Pulses** : `x-chariow-signature` = HMAC-SHA256 des octets BRUTS (contrat officiel chariow.dev « Pulse Security »), clé `whsec_` telle quelle, temps constant, AVANT toute dépense. **Fail-closed** : sans secret NI `CHARIOW_PULSE_OBSERVE=1` → 503 ; l'observation est EXPLICITE (posée au déploiement, à retirer quand le secret est copié). La re-vérification `GET /v1/sales/{id}` reste l'autorité sur le CONTENU. Piège verrouillé par test : jamais re-sérialiser le JSON (`\/` échappés) |
| ✅ C4 | **Repli honnête** : après l'échéance, UNE rafale courte (`CADENCE_RELANCE_MS`, ~30 s) — la réconciliation a pu naître à la dernière seconde — puis : **en recette (`essai`) = PANNE FRANCHE affichée** (aucun e-mail promis — la promesse masquerait ce que la recette existe pour voir) ; en réel = référence + consigne. `?commande=` se VALIDE (UUID) avant d'ouvrir la salle |
| ✅ C5 | **E-mails multipart** (texte brut sur n°1 ET n°2 — un webmail texte-seul recevait un message VIDE) ; **facture DURABLE** : Edge `order-invoice` (jeton dans le CORPS d'un POST, jamais en URL) re-signe l'URL Chariow à la volée — bouton sur TOUS les écrans de `/u/` (l'e-mail la promet dès la naissance ; fenêtre ouverte DANS le geste de clic — Safari bloquait après l'await) ; e-mails tiers absorbés : « **MiMo Global** » au panneau ET au reçu, **clé de licence Chariow expliquée** dans l'e-mail n°1 |
| ✅ B2 | **Configuration APRÈS paiement** : panneau = offre + identité + payer (formulaire config/fichier/bundle SUPPRIMÉ de `modele.html` + jumelle EN **régénérée par le générateur — jamais éditée à la main**, gardée par le test de câblage) ; le pont n'envoie plus RIEN (téléversement, réservation IndexedDB, `docTypeServeur`, gardes fichier : supprimés) ; `/u/` est l'unique point de config — l'écran de dépôt les demandait déjà. `nouvelleCommande` = `{id, cree, doc, offre}` |

**Séquence de déploiement C (exécutée dans l'ordre sûr)** : ① `0094` appliquée (RPC hash + skips + cron gardé) ② secrets Vault créés EN base (`gen_random_bytes`) ③ sonde (hash 64, table, cron posé) ④ Edge `chariow-pulse`/`chariow-reconcile`/`order-invoice`/`job-tick` déployées ⑤ `CHARIOW_PULSE_OBSERVE=1` posé ⑥ fumée 3/3 (reconcile 403 fermé · invoice 404 propre · pulse charabia 200 acquitté). ✅ **Gestes console FAITS (2026-08-15)** : `CHARIOW_PULSE_SECRET` posé depuis le presse-papiers (la valeur n'a jamais transité en clair), `CHARIOW_PULSE_OBSERVE` retiré — **le webhook est fail-closed, signature vérifiée en prod** ; `send_delivery_email: false` vérifié par API sur les 4 produits upgrade.

**✅ Vérification du rail PROUVÉE de bout en bout (2026-08-15, rejeux console — zéro coût, zéro effet)** :
le rejeu de la livraison réelle du 14/08 (onglet « Livraisons » du Pulse) a d'abord **payé** : signature
acceptée (aucun `signature_refusee`), mais `pulse_illisible` — le corps RÉEL des Pulses porte
l'identifiant sous **`sale.id`** (`{ event, sale: { id, … }, store, product, customer }`), forme que
`lirePulse` ne connaissait pas (les formes plates venaient de la documentation ; avant correctif, la
naissance retombait sur la réconciliation, 2 min au lieu de l'instantané). Correctif
[PR #478](https://github.com/pharnos-mvp/pharnos/pull/478) (squash `a5f515c7`, revue adverse : précédence
« premier candidat UTILISABLE », `sale.id` prime sur l'`id` de racine, format `SALE_ID_RE` épinglé,
taille du corps journalisée) + `chariow-pulse` redéployée. **Second rejeu : console `200 · Réussie`,
log Edge `status: "rejeu"`** — signature → lecture `sale.id` → re-vérification API → idempotence, la
chaîne entière sous les yeux. Piège CI appris en chemin : gitleaks scanne TOUS les refs ⇒ empreintes
à épingler sous le hash de branche ET le hash squashé (les deux posées).
