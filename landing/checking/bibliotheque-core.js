...
export const fmtMontant = (n, lang) =>
  n.toLocaleString(lang === 'en' ? 'en-GB' : 'fr-FR').replace(/[\u202F\u00A0]/g, ' ')
 ...
 */
export const prixDouble = (p, lang) => `${fmtMontant(p.eur, lang)} € (${fmtMontant(p.xof, lang)} FCFA)`
...
/**
 * Le fichier à servir pour un document et un pays.
 *
 ...
 * enverrait `vigilances.abmed@gouv.bj` dans un dossier sénégalais.
 *
 * @param {string} slug  `rcp`, `notice`, `etiquetage`.
 * @param {string} pays  Code pays (`bj`, …). Ignoré si le document ne varie pas.
 */
export function fichierModele(slug, pays) {
  ...
  if (!f) throw new Error(`modèle « ${slug} » : aucun fichier pour le pays « ${pays} »`)
  return f
}
...
export function tailleLisible(octets, lang) {
  ...
  if (mo >= 1) return `${fmtMontant(Math.round(mo * 10) / 10, lang)} ${lang === 'en' ? 'MB' : 'Mo'}`
...
export function validerFichier(f) {
  ...
  if (!EXTENSIONS.some((e) => nom.endsWith(e))) return { ok: false, raison: 'extension' }
...
export const estPerimee = (cmd, maintenant) => !cmd || maintenant - cmd.cree > TTL_MS
...
export function nouvelleCommande(p) {
  ...
  if (!MODELES_FICHIERS[p.doc]) throw new Error(`document inconnu « ${p.doc} »`)
