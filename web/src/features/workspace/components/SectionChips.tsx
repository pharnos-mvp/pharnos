...
export function SectionChips({
  ...
  return (
    <nav
      ...
      className="bg-muted/40 flex shrink-0 items-center gap-1 border-t px-1 py-1"
    >
      ...
      <div
        ref={railRef}
        onKeyDown={onKeyDown}
        className="flex min-w-0 flex-1 [scrollbar-width:none] items-center gap-1.5 overflow-x-auto scroll-smooth py-1 [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden"
      >
        {chips.map((c) => (
          <button
            key={c.id}
            ...
            className={cn(
              ...
              c.active
                ? 'border-brand text-brand size-[46px] border-2 text-[12.5px] font-bold shadow-md'
                : 'border-border text-foreground hover:bg-accent size-10 text-[11px]',
            )}
          >
            {c.number}
            {c.flagged ? (
